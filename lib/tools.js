...
const nodeState = require("../repo/fileCreator.js");
...
const koi_contract = "AVmaNm_jiB6X47ker8IkyPriZLY6pr6OEVVOhNlGyEo";
// const bundlerNodes = "http://bundler.openkoi.com:8887";

const bundlerNodes = "http://localhost:8887/submitVote/"; // @abel - I have a gateway set up on this node, I think we can run the server there as well
 ...
 */
  ...
  */
  ...
  async vote(arg) {
    ...
    if (reciept) {
      this.totalVoted += 1;
      await nodeState.setTotalVoted(this.totalVoted);
      this.reciepts.push(reciept);
      await nodeState.setReciept(reciept.data.receipt);
      return reciept;
    }
 ...
 */
  ...
  @userState 
  returns = an object with totalViews of user nfts and user balance
  ...
  async userState() {
    ...
    const userState = { totalViews: 0, koiBalance: balances[this.address] };
    const viewsArray = await this.myContent();
    for (let nftItemDescription of viewsArray) {
      userState.totalViews += nftItemDescription.totalViews;
    }
    return userState;
  }
 ...
 */
  ...
  async _bundlerNode(payload) {
    let signedPayload = await this.signPayload(payload);

    return new Promise(function (resolve) {
      axios
        .post(bundlerNodes, signedPayload)
        .then((res) => {
