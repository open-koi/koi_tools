 ...
 */
  ...
  */
  ...
  async _retrieveAllContent() {
    ...
    let result = contents.filter( res => res.status != "rejected" )
    let clean = result.map(res => res.value)
    console.log('allSettled returned', result, contents.length, result.length, clean.length,  'SUCCESSS?????')
    return clean;
  }
  ...
  async myContent() {
    ...
    for (let txId in registerRecords) {
        try {
          ...
          console.log('error retrieving content with id ', txId)
        }
    }
