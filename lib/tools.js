...
const { Query } = require("@kyve/query");
...
//const koi_contract = "KylO-AMt1h8Cn5nWZ1h1s5gh4rahoNONW5VyN0BBZzo";
//const koi_contract = "j6fu4HX4zylBaaI1yqLumC2eLn4HBLZhOZOUwYCVeMs"; // main_contract
//const koi_contract = "Unal8PLXXYOXSlRbdWXFPzwjt6YrG6i8h-BO1Vh40pU"; // main_contract
//const koi_contract = "SRsbQLZ-Mkv8-nHfdgjpw5ePe3xqCz2nB_vKI2L6C-c";
//const koi_contract = "ldW53WDyrRSWWXge2G3PKG8Z3ldiRXHXo8306ewr-QQ"; // attentionReward
// const koi_contract = "AjnJO8XkSaQxnF_gI6ZKK6QneWUsX82dNFCKWgnHMpQ"; // attention from testweave
...
class koi {
  ...
  */
  ...
  /*registerTask to main contract */
  async registerTask(arg) {
    let input = {
      function: "registerTask",
      taskId: arg.taskId,
      taskname: arg.taskName,
      taskTxId: arg.taskTxId,
      KOI_Reward: arg.koiReward,
    };
    let result = await this._interactWrite(input);
    return result;
  }
  async deregisterTask(taskId) {
    console.log(taskId);
    let input = {
      function: "deregisterTask",
      taskId: taskId,
    };
    let result = await this._interactWrite(input);
    return result;
 ...
 */
  ...
  async vote(arg) {
    ...
    if (reciept) {
      if (reciept.status == 200) {
        if (reciept.data.message == "success") {
          this.totalVoted += 1;
        ...
        if (reciept.data.message == "duplicate") {
          this.totalVoted += 1;
          await this._db();
        ...
        this.totalVoted += 1;
        await this._db();
        return { message: reciept.message };
      }
    }
  ...
  */
  ...
  async contentView(contentTxId, address, state) {
    // const state = await this.getContractState();
  ...
  async _retrieveAllContent() {
    ...
    let contentViewPromises = txIdArr.map((txId) =>
      this.contentViewWrapper(txId, registerRecords[txId], state)
    );
  ...
  contentViewWrapper(txId, address, state) {
    return new Promise((resolve, reject) => {
      this.contentView(txId, address, state)
        .then((value) => {
          resolve(value);
        })
        .catch((e) => {
  ...
  async myContent() {
    ...
    const contents = [];
    let my_data = data.filter((_item) => _item.owner === this.address);
 ...
 */
  ...
  async readNftState(txId, Arweave) {
    return new Promise(function (resolve, reject) {
      smartweave
        .readContract(Arweave, txId)
        .then((state) => {
          resolve(state);
        })
        .catch((err) => {
