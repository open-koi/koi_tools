 ...
 */
  ...
  async stake(qty) {
    ...
    let { txId, finalState } = await this._interactWrite(input);
    const result = await this._errorFromContract(txId, finalState);
    return result;
  }
  /* error Message form contract from predictate State a reusable function for all function that interact with contract , this function retuns an object with message and txId */

  async _errorFromContract(txId, finalState) {
    let result = {};
    if (finalState.type == "ok") {
      result = { message: "sucess", txId: txId };
    } else {
      result = { message: finalState.result, txId: txId };
    }
    return result;
  }
  ...
  async vote(arg) {
    ...
    if (tx) {
      if (state.type == "ok") {
        ...
        return { message: state.result, error: true };
      }
    }
  ...
  */
  ...
  async _retrieveAllContent() {
    ...
    const passedNft = clean.filter((element) => {
      let txId = "NI9OY-Rzj4TjS5s4W-DFyRoY2Vz4884OhSYdjFDl1uY";
      axios
