...
class koi {
  constructor() {
    ...
    console.log('Initialized a Koi Node with the smart contract: ' + koi_contract);
 ...
 */
  ...
  */
  async registerData(txId, owner, wallet) {
    let input = {
    ...
    // return new Promise(function (resolve, reject) {
    //   smartweave
    //     .interactWrite(testArweave, wallet, koi_contract, input)
    //     .then((txId) => {
    //       resolve(txId);
    //     })
    //     .catch((err) => {
  ...
  async registerDataDryRun(txId, owner, tx,state) {
    ...
    if(finalState.type!='exception'){
      await redisSetAsync('TempPredictedState', JSON.stringify(finalState));
      return finalState;
    }else{
      console.error("EXACEPTIOBN",finalState)
    }
    return state
    // this._interactWrite(input)
  }
  ...
  */
  ...
  async contentView(contentTxId, state) {
    ...
    const rewardReport = _.get(state, 'stateUpdate.trafficLogs.rewardReport', []);
  ...
  async _retrieveAllContent() {
    ...
    let contentViewPromises = txIdArr.map(txId => this.contentViewWrapper(txId, state));
...
*/
  async _interactWrite(input) {
    ...
    let latestContractState = await redisGetAsync('TempCurrentState');
    ...
    return new Promise(function(resolve, reject) {
      smartweave
        ...
        .then(async txId => {
          ...
          await this.recalculatePredictedState(wallet, latestContractState);
 ...
 */
  ...
  async recalculatePredictedState(wallet, latestContractState) {
    ...
    for (let i = 0; i < pendingStateArray.length; i++) {
      console.log(`Pending Transaction ${i + 1}`, pendingStateArray[i].status);
      if (i == 0) {
    ...
    if (finalState) await redisSetAsync('TempPredictedState', JSON.stringify(finalState));
  }
}
...
async function checkPendingTransactionStatus() {
  ...
  pendingStateArray = JSON.parse(pendingStateArray);
  for (let i = 0; i < pendingStateArray.length; i++) {
    let arweaveTxStatus = await arweave.transactions.getStatus(pendingStateArray[i].txId);
