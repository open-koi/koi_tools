...
const koi_contract = "qGNZGZBNv5JffEPkkuYpHEOEtZYHLhrRrRygAjdanqw";
 ...
 */
  ...
  */
  ...
  async contentView(contentTxId, state, addNfState = false) {
  ...
  async myContent() {
    ...
    for (let txId in registerRecords) {
      if (registerRecords[txId] == this.address) {
        let nftInfo = await this.contentView(txId, state.data, true);
        if (nftInfo !== null) {
