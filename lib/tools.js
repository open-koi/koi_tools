...
const mnemonic_keys = require("arweave-mnemonic-keys")
...
class koi {
  ...
  */
  ...
  /*
    @loadWallet // loads wallet key from source.
    Returns the key as an object.
    source: // wallet key file location or mnemonic
  */
  async loadWallet(source) {
    switch(typeof source) {
      case "string":
        this.mnemonic = source;
        this.wallet = await mnemonic_keys.getKeyFromMnemonic(source); // This can take a minute
        break;
      case "object":
        this.wallet = source;
        break;
      default:
        this.wallet = await loadFile(source);
    } 
  ...
  /*
    @loadWallet // loads wallet for node Simulator key from source and initialize ndb.
    Returns the key as an object.
    source: // wallet key file location or mnemonic
  */

  async nodeLoadWallet(source) {
    await this.loadWallet(source);
  ...
  /*
    ...
    use_mnemonic: // [false] flag on whether to use a mnemonic for key generation 
  */
  async generateWallet(use_mnemonic) {
    if (use_mnemonic === true)
      this.mnemonic = await mnemonic_keys.generateMnemonic(); // This can take a minute
    let key = use_mnemonic === true ?
      await mnemonic_keys.getKeyFromMnemonic(this.mnemonic) : // This can take a minute
      await arweave.wallets.generate();
    if (!key) return "failed to create wallet";
    this.wallet = key;
    await this.getWalletAddress();
    return this.wallet ? true : "wallet not set";  // Not sure why we have to check again
  }
    ...
    return new Promise((resolve, reject) => {
      crypto.generateKeyPair(
        ...
        (err, _publicKey, privateKey) => {
          ...
          if (err) reject(err);
          resolve(Arweave.crypto.pemToJWK(privateKey));
        }
      )
    })
  }
